...
public class Files {
    ...
    public static long flushTo(InputStream is, File dest) throws IOException {
        if (dest.exists() && dest.isDirectory()) {
            throw new IllegalArgumentException("not a file : " + dest); //$NON-NLS-1$
        }
        ...
        if (Environment.MEDIA_MOUNTED.equals(state) && sourceFolder.exists() && targetFolder.exists()) {
            String[] filenames = sourceFolder.list(new FilenameFilter() {
                ...
                public boolean accept(File dir, String filename) {
                    // fsf db files only
                    if (filename.startsWith("fsf") && filename.endsWith(".db")) {
                        return true;
                    }
                    return false;
                }
                ...
                public boolean accept(File dir, String filename) {
                    try {
                        if (filename.endsWith(".bak")) {
                            ...
                            Date backupDate = backupDateFmt.parse(backupTimeString);
